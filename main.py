# === main.py (versione debug stabile) =========================================
import os
import time
import json
import logging
...
# ⚠️ Render non ha Europe/Rome: usiamo UTC (sempre presente)
APP_TZ = tz.gettz("UTC")

CAPITAL_BASE_URL = _clean(os.getenv("CAPITAL_BASE_URL", "https://api-capital.backend-capital.com"))
CAPITAL_API_KEY = _clean(os.getenv("CAPITAL_API_KEY"))
CAPITAL_IDENTIFIER = _clean(os.getenv("CAPITAL_IDENTIFIER"))   # email O clientId
CAPITAL_PASSWORD = _clean(os.getenv("CAPITAL_PASSWORD"))
...
if not all([CAPITAL_API_KEY, CAPITAL_IDENTIFIER, CAPITAL_PASSWORD, TELEGRAM_TOKEN, TELEGRAM_CHAT_ID]):
    logger.warning("Mancano una o più ENV (CAPITAL_* o TELEGRAM_*).")

# ------------------------------------------------------------------------------
# ASSET LIST – termini multipli per trovare l'EPIC
# ------------------------------------------------------------------------------
...
class CapitalAPI:
    ...
    def _auth_headers(self) -> Dict[str, str]:
        h = {"X-CAP-API-KEY": self.api_key}
        if self.cst and self.sec_token:
            h.update({"CST": self.cst, "X-SECURITY-TOKEN": self.sec_token})
        return h
    ...
    def login(self) -> None:
        ...
        # --- DEBUG sicuro (mascherato) ---
        id_type = "email" if "@" in self.identifier else "clientId"
        logger.info("[CapitalAPI] login → %s (id=%s, base=%s)", url, id_type, self.base_url)
        logger.info("[CapitalAPI] key=%s pwd_len=%d",
                    (self.api_key[:3] + "…" + self.api_key[-3:]) if self.api_key else "None",
    ...
    def _get(self, path: str, params: Dict = None) -> requests.Response:
        ...
        if r.status_code == 401:
            logger.warning("[CapitalAPI] 401 → re-login…")
            self.login()
            r = self.session.get(url, headers=self._auth_headers(), params=params or {}, timeout=30)
        self.last_touch = time.time()
        return r
    ...
    def search_markets(self, term: str) -> List[Dict]:
        ...
        if r.status_code != 200:
            logger.warning("Markets search failed for %s: %s %s", term, r.status_code, r.text)
            return []
        ...
        for term in terms + [_sanitize(t) for t in terms]:
            try:
                ...
                if mkts:
                    ...
                    logger.info("Mappato %s (%s) → %s", label, term, epic)
                    break
            except Exception as e:
                logger.warning("Search errore per %s/%s: %s", label, term, e)
        if not epic:
            logger.warning("Nessun epic trovato per %s", label)
        else:
            mapping[label] = epic
    return mapping

# ------------------------------------------------------------------------------
# INDICATORI/STRATEGIA (invariati)
# ------------------------------------------------------------------------------
...
def send_telegram(msg: str) -> None:
    try:
        ...
        if r.status_code != 200:
            logger.warning("Telegram send errore: %s %s", r.status_code, r.text)
    except Exception as e:
...
class Scanner:
    ...
    def scan_once(self, tf_label: str, labels: List[str]):
        ...
        for label in labels:
            ...
            if not epic:
                logger.warning("Skip %s: epic non trovato", label)
                continue
            try:
                raw = self.api.get_prices(epic, res, max_points=400)
                df = prices_to_df(raw)
                if df is None or len(df) < 60:
                    logger.info("%s %s: dati insufficienti", label, tf_label)
                    continue
                sig = detect_pre_rally(df)
                if sig:
                    now = datetime.now(APP_TZ).strftime("%Y-%m-%d %H:%M")
                    msg = (f"<b>[CAPITAL][{tf_label}] {label} → {sig['side']}</b>\n"
                    ...
                    logger.info("%s %s: nessun setup", label, tf_label)
            except Exception as e:
                logger.exception("Errore scan %s %s: %s", label, tf_label, e)
...
def _startup():
    """Parte all'avvio dell'app (chiamato da Uvicorn)."""
    ...
    api.login()  # <-- se qui fallisce 401 vediamo il log dettagliato sopra
    ...
    try:
        ...
        logger.exception("Scan di prova fallito: %s", e)

@app.get("/")
@app.get("/health")
def health():
    id_type = "email" if "@" in CAPITAL_IDENTIFIER else "clientId"
    return {
        "status": "ok",
        "time": datetime.now(APP_TZ).isoformat(),
        "base": CAPITAL_BASE_URL,
        "id_type": id_type
    }
# === END ======================================================================
